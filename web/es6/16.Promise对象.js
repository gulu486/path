...
// 创造了一个Promise实例
const promise = new Promise((resolve, reject) => { // 该函数的两个参数分别是resolve和reject。它们是两个函数
  // ... some code
  ...
  // /* 异步操作成功 */ ? resolve(value) : reject(error)
});
...
function timeout(ms) {
  return new Promise((resolve, reject) => {
    ...
    setTimeout(resolve, ms, 'done');//第三个参数'done',可选。传给执行函数的其他参数
  });
}
...
promise.then(() => {
  console.log('resolved.');//所有同步任务执行完才会执行
});
console.log('Hi!');
// Promise
// Hi!
...
const p1 = new Promise(function (resolve, reject) {
  setTimeout(() => reject(new Error('fail')), 3000)//3 秒之后变为rejected
})
const p2 = new Promise(function (resolve, reject) {
...
// 理由是第二种写法可以捕获前面then方法执行中的错误，也更接近同步的写法（try/catch）。因此，建议总是使用catch()方法，而不使用then()方法的第二个参数。
// good
promise
...
promise.then(function (value) { console.log(value) });//一般总是建议，Promise 对象后面要跟catch()方法
// ok
// Uncaught Error: test
...
someAsyncThing().then(function() {
  ...
  // 下面一行会报错，因为y没有声明
  y + 2;
}).catch(function(error) {//第二个catch()方法用来捕获前一个catch()方法抛出的错误。
